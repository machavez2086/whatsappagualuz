...
use Sonata\UserBundle\Controller\RegistrationFOSUser1Controller as Base;
 ...
 */
class RegistrationFOSUser1Controller extends Base
{
    /**
...
//            dump($user->getId());die;
            $myUser = $em->getRepository('ApplicationSonataUserBundle:User')->find($user->getId());
            $userGroup = $em->getRepository('ApplicationSonataUserBundle:Group')->findByName("Usuarios");
            if(count($userGroup ) > 0){
                $myUser->addGroup($userGroup[0]);
                $myUser->setTimezone("America/Mexico_City");
                $em->persist($myUser);
                $em->flush();
            }
    ...
    public function confirmAction($token)
    {
        ...
        $this->authenticateUser($user, $response);
        $this->setFlash('fos_user_success', 'Su usuario ha sido confirmado satisfactoriamente. Para culminar los pasos de registro por favor edite su perfil.');
