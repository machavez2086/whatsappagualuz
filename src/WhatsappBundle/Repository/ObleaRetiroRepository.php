...
class ObleaRetiroRepository extends EntityRepository {
   public function countObleaThisDay($configuration){
        $day = new \DateTime("today");
        $day->setTime(0, 0, 0);
        $em = $this->getEntityManager();
        $dql = 'SELECT count(s) FROM WhatsappBundle:ObleaRetiro s where s.createdAt > :createdAt and s.configuration =:configuration';
...
//        $day->setTime(0, 0, 0);
        $em = $this->getEntityManager();
        $dql = 'SELECT count(s) FROM WhatsappBundle:ObleaRetiro s where s.createdAt >= :ini and s.createdAt <= :finalDay  and s.configuration =:configuration';
        $consulta = $em->createQuery($dql);
