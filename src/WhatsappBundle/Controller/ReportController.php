...
class ReportController extends Controller {
    ...
    /**
     * @Route("/admin/report/report_summary", name="report_summary" )
     */
    public function reportSummaryAction() {
//        $this->get('session')->set("menu", "report_summary");
        $request = $this->getRequest();
        $month = $request->query->get('month');
        $em = $this->getDoctrine()->getManager();
        $isCreated = $em->getRepository('WhatsappBundle:Configuration')->findByCompany("Default");
        $configuration = $isCreated[0];
        $lastTicket = $em->getRepository('WhatsappBundle:Peticion')->findLastTicket($configuration->getId());
        $minutesSinceLastTicket = 0;
        $minutesSinceLastTicket = "";
        if (count($lastTicket) > 0) {
            $maxDate = new \DateTime("now");
            $since_start = $maxDate->diff($lastTicket[0]->getCreatedAt());
...
//            dump($minutesSinceLastTicket);die;
        ...
        // Buscar cantidad de mensajes enviados por whatsapp, cantidad de mensajes recibidos por whatsapp
        // Hacer un grafico de pastel con esos datos
        // Buscar estos datos en cada mes del año para una grafica de barras
 
        $timestamp = strtotime("first day of this month");
        $ini = new \DateTime();
        //$day->setTimestamp(strtotime("-3 month", $timestamp)); 
        if($month != null){
            $ini->setTimestamp(strtotime($month." month", $timestamp));
        }
        else{
            $ini->setTimestamp($timestamp);
        }
        $ini->setTime(0, 0, 0);
        $finalDay = clone $ini;
        $finalDay->modify('last day of this month');
        $finalDay->setTime(23, 59, 59);
//        dump($ini);
//        dump($finalDay);die;
        $cantTicketsThisDay = $em->getRepository('WhatsappBundle:Peticion')->countTicketsThisDay($configuration->getId());
        $cantTicketsThisWeek = $em->getRepository('WhatsappBundle:Peticion')->countTicketsThisWeek($configuration->getId());
        $cantTicketsThisMonth = $em->getRepository('WhatsappBundle:Peticion')->countTicketsThisMonth($configuration->getId(), $ini, $finalDay);
        $cantTicketsOpened = $em->getRepository('WhatsappBundle:Peticion')->countTicketsOpened($configuration->getId());
        $openedTickets = $em->getRepository('WhatsappBundle:Peticion')->ticketsOpeneds($configuration->getId());
        
        $cantObleasRetiroDia = $em->getRepository('WhatsappBundle:ObleaRetiro')->countObleaThisDay($configuration->getId());
        $cantObleasRetiroWeek = $em->getRepository('WhatsappBundle:ObleaRetiro')->countObleaThisWeek($configuration->getId());
        $cantObleasRetiroMonth = $em->getRepository('WhatsappBundle:ObleaRetiro')->countObleaThisMonth($configuration->getId(), $ini, $finalDay);
        $cantObleasEnvioDia = $em->getRepository('WhatsappBundle:ObleaEnvio')->countObleaThisDay($configuration->getId());
        $cantObleasEnvioWeek = $em->getRepository('WhatsappBundle:ObleaEnvio')->countObleaThisWeek($configuration->getId());
        $cantObleasEnvioMonth = $em->getRepository('WhatsappBundle:ObleaEnvio')->countObleaThisMonth($configuration->getId(), $ini, $finalDay);
        
        $cantTicketSendedAreaDia = $em->getRepository('WhatsappBundle:TicketSendedArea')->countTicketSendedAreaThisDay($configuration->getId());
        $cantTicketSendedAreaWeek = $em->getRepository('WhatsappBundle:TicketSendedArea')->countTicketSendedAreaThisWeek($configuration->getId());
        $cantTicketSendedAreaMonth = $em->getRepository('WhatsappBundle:TicketSendedArea')->countTicketSendedAreaThisMonth($configuration->getId(), $ini, $finalDay);
        $cantTicketSendedAreaAnsweredDia = $em->getRepository('WhatsappBundle:TicketSendedArea')->countTicketSendedAreaAnsweredThisDay($configuration->getId());
        $cantTicketSendedAreaAnsweredWeek = $em->getRepository('WhatsappBundle:TicketSendedArea')->countTicketSendedAreaAnsweredThisWeek($configuration->getId());
        $cantTicketSendedAreaAnsweredMonth = $em->getRepository('WhatsappBundle:TicketSendedArea')->countTicketSendedAreaAnsweredThisMonth($configuration->getId(), $ini, $finalDay);
        
        
        return $this->render('WhatsappBundle:Report:report_summary.html.twig', array(
            ...
            'cantObleasRetiroDia' => $cantObleasRetiroDia,
            'openedTickets' => $openedTickets,
            'cantObleasRetiroDia' => $cantObleasRetiroDia,
            'cantObleasRetiroWeek' => $cantObleasRetiroWeek,
            'cantObleasRetiroMonth' => $cantObleasRetiroMonth,
            'cantObleasEnvioDia' => $cantObleasEnvioDia,
            'cantObleasEnvioWeek' => $cantObleasEnvioWeek,
            'cantObleasEnvioMonth' => $cantObleasEnvioMonth,
            
            'cantTicketSendedAreaDia' => $cantTicketSendedAreaDia,
            'cantTicketSendedAreaWeek' => $cantTicketSendedAreaWeek,
            'cantTicketSendedAreaMonth' => $cantTicketSendedAreaMonth,
            'cantTicketSendedAreaAnsweredDia' => $cantTicketSendedAreaAnsweredDia,
            'cantTicketSendedAreaAnsweredWeek' => $cantTicketSendedAreaAnsweredWeek,
            'cantTicketSendedAreaAnsweredMonth' => $cantTicketSendedAreaAnsweredMonth,
            'actualMonth' => $this->getActualMonth($ini),
    ...
    /**
     * @Route("/admin/report/report_month", name="report_month" )
     */
    public function reportMonthAction() {
        ...
        $ini->setTime(0, 0, 0);
        $finalDay = clone $ini;
        $iniThisMonth = clone $ini;
        $finalDay->modify('last day of this month');
        $finalDay->setTime(23, 59, 59);
        
//        $this->get('session')->set("menu", "report_month");
        $em = $this->getDoctrine()->getManager();
        //Mostrar los tickets del mes de forma ordenada por fecha
        //graficos de linea y pastel por cada uno
        //Graficos por tipo
        //Graficos por categoria
        //Graficos por actitud del cliente
        //Graficos por Producto
        //Graficos por Motivo del reclamo
        
        $isCreated = $em->getRepository('WhatsappBundle:Configuration')->findByCompany("Default");
        $configuration = $isCreated[0];
        
        //graficos de pastel
        $ticketsThisMonth = $em->getRepository('WhatsappBundle:Peticion')->findTicketsThisMonth($configuration->getId(), $ini, $finalDay);
        $cantTicketsThisMonth = $em->getRepository('WhatsappBundle:Peticion')->countTicketsThisMonth($configuration->getId(), $ini, $finalDay);
        //grupos por tipo
        $groupTicketsThisMonthByPeticionType = $em->getRepository('WhatsappBundle:Peticion')->findGroupTicketsThisMonthByPeticionType($configuration->getId(), $ini, $finalDay);
        $sliced = true;
        $piegroupTicketsThisMonthByPeticionType = array();
        foreach ($groupTicketsThisMonthByPeticionType as $value) {
            $stdObject = new \stdClass();
            $stdObject->name = $value["name"];
            $stdObject->y = $value["total"]*100/$cantTicketsThisMonth;
            $stdObject->sliced = $sliced;
            $stdObject->selected = $sliced;
            $sliced = false;
            $piegroupTicketsThisMonthByPeticionType[] = $stdObject;
        }
        
        //grupos por categorias
        $groupTicketsThisMonthByCategory = $em->getRepository('WhatsappBundle:Peticion')->findGroupTicketsThisMonthByCategory($configuration->getId(), $ini, $finalDay);
        $sliced = true;
        $piegroupTicketsThisMonthByCategory = array();
        foreach ($groupTicketsThisMonthByCategory as $value) {
            $stdObject = new \stdClass();
            $stdObject->name = $value["name"];
            $stdObject->y = $value["total"]*100/$cantTicketsThisMonth;
            $stdObject->sliced = $sliced;
            $stdObject->selected = $sliced;
            $sliced = false;
            $piegroupTicketsThisMonthByCategory[] = $stdObject;
        }
        
        //grupos por actitud del cliente
        $groupTicketsThisMonthByClientActitud = $em->getRepository('WhatsappBundle:Peticion')->findGroupTicketsThisMonthByClientActitud($configuration->getId(), $ini, $finalDay);
        $sliced = true;
        $piegroupTicketsThisMonthByClientActitud = array();
        foreach ($groupTicketsThisMonthByClientActitud as $value) {
            $stdObject = new \stdClass();
            $stdObject->name = $value["name"];
            $stdObject->y = $value["total"]*100/$cantTicketsThisMonth;
            $stdObject->sliced = $sliced;
            $stdObject->selected = $sliced;
            $sliced = false;
            $piegroupTicketsThisMonthByClientActitud[] = $stdObject;
        }
        
        //grupos por Producto
        $groupTicketsThisMonthByProduct = $em->getRepository('WhatsappBundle:Peticion')->findGroupTicketsThisMonthByProduct($configuration->getId(), $ini, $finalDay);
        $sliced = true;
        $piegroupTicketsThisMonthByProduct = array();
        foreach ($groupTicketsThisMonthByProduct as $value) {
            $stdObject = new \stdClass();
            $stdObject->name = $value["name"];
            $stdObject->y = $value["total"]*100/$cantTicketsThisMonth;
            $stdObject->sliced = $sliced;
            $stdObject->selected = $sliced;
            $sliced = false;
            $piegroupTicketsThisMonthByProduct[] = $stdObject;
        }
        
        //grupos por Motivo del reclamo
        $groupTicketsThisMonthByMotive = $em->getRepository('WhatsappBundle:Peticion')->findGroupTicketsThisMonthByMotive($configuration->getId(), $ini, $finalDay);
        $sliced = true;
        $piegroupTicketsThisMonthByMotive = array();
        foreach ($groupTicketsThisMonthByMotive as $value) {
            $stdObject = new \stdClass();
            $stdObject->name = $value["name"];
            $stdObject->y = $value["total"]*100/$cantTicketsThisMonth;
            $stdObject->sliced = $sliced;
            $stdObject->selected = $sliced;
            $sliced = false;
            $piegroupTicketsThisMonthByMotive[] = $stdObject;
        }
        
        //Graficos de lineas
        $groupTicketsThisMonthByPeticionTypeLine = array();
        foreach ($groupTicketsThisMonthByPeticionType as $value) {
            if(!array_key_exists($value["name"], $groupTicketsThisMonthByPeticionTypeLine))
                $groupTicketsThisMonthByPeticionTypeLine[$value["name"]] = array();  
        }
        
        $groupTicketsThisMonthByCategoryLine = array();
        foreach ($groupTicketsThisMonthByCategory as $value) {
            if(!array_key_exists($value["name"], $groupTicketsThisMonthByCategoryLine))
                $groupTicketsThisMonthByCategoryLine[$value["name"]] = array();  
        }
        
        $groupTicketsThisMonthByClientActitudLine = array();
        foreach ($groupTicketsThisMonthByClientActitud as $value) {
            if(!array_key_exists($value["name"], $groupTicketsThisMonthByClientActitudLine))
                $groupTicketsThisMonthByClientActitudLine[$value["name"]] = array();  
        }
        
        $groupTicketsThisMonthByProductLine = array();
        foreach ($groupTicketsThisMonthByProduct as $value) {
            if(!array_key_exists($value["name"], $groupTicketsThisMonthByProductLine))
                $groupTicketsThisMonthByProductLine[$value["name"]] = array();  
        }
        
        $groupTicketsThisMonthByMotiveLine = array();
        foreach ($groupTicketsThisMonthByMotive as $value) {
            if(!array_key_exists($value["name"], $groupTicketsThisMonthByMotiveLine))
                $groupTicketsThisMonthByMotiveLine[$value["name"]] = array();  
        }
        $stdObjectTypeLineArrayCategories = array();
        while ($ini < $finalDay) {  
            $stdObjectTypeLineArrayCategories[]=$ini->format("d");
            $insidenciasPorDiaGraphic[] = 1;
            $final = clone $ini;
            $final->setTime(23, 59, 59);
            if ($final > $finalDay){
                $final = $finalDay;
            }
            foreach (array_keys($groupTicketsThisMonthByPeticionTypeLine) as $value) {
                $cant = $em->getRepository('WhatsappBundle:Peticion')->countGroupTicketsByDatesByPeticionType($configuration->getId(), $ini, $final, $value);
                $groupTicketsThisMonthByPeticionTypeLine[$value][] = round($cant,2);
            }
            foreach (array_keys($groupTicketsThisMonthByCategoryLine) as $value) {
                $cant = $em->getRepository('WhatsappBundle:Peticion')->countGroupTicketsByDatesByCategory($configuration->getId(), $ini, $final, $value);
                $groupTicketsThisMonthByCategoryLine[$value][] = round($cant,2);
            }
            foreach (array_keys($groupTicketsThisMonthByClientActitudLine) as $value) {
                $cant = $em->getRepository('WhatsappBundle:Peticion')->countGroupTicketsByDatesByClientActitud($configuration->getId(), $ini, $final, $value);
                $groupTicketsThisMonthByClientActitudLine[$value][] = round($cant,2);
            }
            foreach (array_keys($groupTicketsThisMonthByProductLine) as $value) {
                $cant = $em->getRepository('WhatsappBundle:Peticion')->countGroupTicketsByDatesByProduct($configuration->getId(), $ini, $final, $value);
                $groupTicketsThisMonthByProductLine[$value][] = round($cant,2);
            }
            foreach (array_keys($groupTicketsThisMonthByMotiveLine) as $value) {
                $cant = $em->getRepository('WhatsappBundle:Peticion')->countGroupTicketsByDatesByMotive($configuration->getId(), $ini, $final, $value);
                $groupTicketsThisMonthByMotiveLine[$value][] = round($cant,2);
            }
            $ini->modify("+1 day");
            $ini->setTime(0, 0, 0);
        }
        $stdObjectTypeLineArray = array();
        foreach ($groupTicketsThisMonthByPeticionTypeLine as $key => $value) {
            $stdObjectTypeLine = new \stdClass();
            $stdObjectTypeLine->name = $key;
            $stdObjectTypeLine->data = $value;
            $stdObjectTypeLineArray[] = $stdObjectTypeLine;
        }
        
        $stdObjectCategoryArray = array();
        foreach ($groupTicketsThisMonthByCategoryLine as $key => $value) {
            $stdObjectCategory = new \stdClass();
            $stdObjectCategory->name = $key;
            $stdObjectCategory->data = $value;
            $stdObjectCategoryArray[] = $stdObjectCategory;
        }
        
        $stdObjectClientActitudArray = array();
        foreach ($groupTicketsThisMonthByClientActitudLine as $key => $value) {
            $stdObjectClientActitud = new \stdClass();
            $stdObjectClientActitud->name = $key;
            $stdObjectClientActitud->data = $value;
            $stdObjectClientActitudArray[] = $stdObjectClientActitud;
        }
        
        $stdObjectProductArray = array();
        foreach ($groupTicketsThisMonthByProductLine as $key => $value) {
            $stdObjectProduct = new \stdClass();
            $stdObjectProduct->name = $key;
            $stdObjectProduct->data = $value;
            $stdObjectProductArray[] = $stdObjectProduct;
        }
        
        $stdObjectMotiveArray = array();
        foreach ($groupTicketsThisMonthByMotiveLine as $key => $value) {
            $stdObjectMotive = new \stdClass();
            $stdObjectMotive->name = $key;
            $stdObjectMotive->data = $value;
            $stdObjectMotiveArray[] = $stdObjectMotive;
        }
        
//        dump($groupTicketsThisMonthByPeticionTypeLine);die;
        
        return $this->render('WhatsappBundle:Report:report_month.html.twig', array(
            
            'piegroupTicketsThisMonthByPeticionType' => $piegroupTicketsThisMonthByPeticionType,
            'piegroupTicketsThisMonthByCategory' => $piegroupTicketsThisMonthByCategory,
            'piegroupTicketsThisMonthByClientActitud' => $piegroupTicketsThisMonthByClientActitud,
            'piegroupTicketsThisMonthByProduct' => $piegroupTicketsThisMonthByProduct,
            'piegroupTicketsThisMonthByMotive' => $piegroupTicketsThisMonthByMotive,
            'stdObjectTypeLineArrayCategories' => $stdObjectTypeLineArrayCategories,
            'stdObjectTypeLineArray' => $stdObjectTypeLineArray,
            'stdObjectCategoryArray' => $stdObjectCategoryArray,
            'stdObjectClientActitudArray' => $stdObjectClientActitudArray,
            'stdObjectProductArray' => $stdObjectProductArray,
            'stdObjectMotiveArray' => $stdObjectMotiveArray,
            'actualMonth' => $this->getActualMonth($iniThisMonth),
            'nextMonth' => intval($month)+1,
            'prevMonth' => intval($month)-1,
    ...
    /**
     * @Route("/admin/report/use_statics", name="use_statics" )
     */
    public function useStaticsAction() {
        $request = $this->getRequest();
        $month = $request->query->get('month');
        $timestamp = strtotime("first day of this month");
        $ini = new \DateTime();
...
//        $ini->setTimestamp($timestamp);
//        $ini->setTime(0, 0, 0);
        $cantConversationsThisMonth = $em->getRepository('WhatsappBundle:Conversation')->countBetwenDates($ini, $finalDay);
        $groupConversationsThisMonthByConversationType = $em->getRepository('WhatsappBundle:Conversation')->findGroupTicketsThisMonthByConversationType($configuration->getId(), $ini, $finalDay);
        $sliced = true;
        $piegroupConversationsThisMonthByConversationType = array();
        foreach ($groupConversationsThisMonthByConversationType as $value) {
            $stdObject = new \stdClass();
            $stdObject->name = $value["name"];
            $stdObject->y = $value["total"]*100/$cantConversationsThisMonth;
            $stdObject->sliced = $sliced;
            $stdObject->selected = $sliced;
            $sliced = false;
            $piegroupConversationsThisMonthByConversationType[] = $stdObject;
        }
        
        $countFromMeThisMonth = $em->getRepository('WhatsappBundle:Message')->countMessagesFromMe($ini, $finalDay);
        $countNotFromMeThisMonth = $em->getRepository('WhatsappBundle:Message')->countMessagesNotFromMe($ini, $finalDay);
        $countThisMonth = $em->getRepository('WhatsappBundle:Message')->countMessages($ini, $finalDay);
//        dump($countFromMeThisMonth);
//        dump($countNotFromMeThisMonth);
//        dump($countThisMonth);
//        dump($cantConversationsThisMonth);
        
        $conversationsCountGroupByType = $em->getRepository('WhatsappBundle:Conversation')->countGroupByType($ini, $finalDay);
        $countTicketsDistintWhatsappGroup = $em->getRepository('WhatsappBundle:Peticion')->countTicketsDistintWhatsappGroup($configuration->getId(), $ini, $finalDay);
        $cantClientsOfTheMonth = count($countTicketsDistintWhatsappGroup);
//        dump($conversationsCountGroupByType);
        
//        dump($countTicketsDistintWhatsappGroup);die;
        
//        dump($piegroupConversationsThisMonthByConversationType);die;
        //
        
        return $this->render('WhatsappBundle:Report:use_statics.html.twig', array(
            'piegroupConversationsThisMonthByConversationType' => $piegroupConversationsThisMonthByConversationType,
            'cantConversationsThisMonth' => $cantConversationsThisMonth,
            'groupConversationsThisMonthByConversationType' => $groupConversationsThisMonthByConversationType,
            'countFromMeThisMonth' => $countFromMeThisMonth,
            'countNotFromMeThisMonth' => $countNotFromMeThisMonth,
            'countThisMonth' => $countThisMonth,
            'countTicketsDistintWhatsappGroup' => $countTicketsDistintWhatsappGroup,
            'cantClientsOfTheMonth' => $cantClientsOfTheMonth,
//            'cantTicketsThisDay' => $cantTicketsThisDay,
//            'cantTicketsThisWeek' => $cantTicketsThisWeek,
//            'cantTicketsThisMonth' => $cantTicketsThisMonth,
//            'cantTicketsOpened' => $cantTicketsOpened,
...
//       die;
    ...
    public function getActualMonth($ini){
        $months = array(
            "01" => "Enero",
            "02" => "Febrero",
            "03" => "Marzo",
...
//        dump($ini->format("m"));die;
        return $months[$ini->format("m")]." ".$ini->format("Y");
    }
