...
class WhatsappGroupCRUDController extends Controller {
    ...
    private function is_super() {
     ...
     public function getSupportMemberByConfigurationAction(Request $request) {
        ...
        if ($configurationId) {
            ...
            $ticket = $em->getRepository('WhatsappBundle:WhatsappGroup')->find($groupId);
            $grupos = $configuration->getSupportMembers();
//            if(count($grupos) > 0)
//                $selected = $grupos[0]->getId();
            foreach ($grupos as $res) {
                if(!$groupId && $selectFirst){
...
//                    $selected = $res->getId();
                    ...
                    if($groupId){
                        if($res == $ticket->getResponsableSupportMember()){
